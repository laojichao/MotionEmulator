...
class EmulationCardAdapter(
    fragment: Fragment,
    val map: UnifiedMapFragment
) : FragmentStateAdapter(fragment) {

    private val emulations = ArrayList<String>()
    ...
    override fun createFragment(position: Int): Fragment {
        ...
        return EmulationControlFragment()
    }

    override fun getItemCount(): Int = emulations.size + 1

    override fun getItemId(position: Int): Long {
        return if (position <= 0) 0L
        else emulations[position - 1].hashCode().toLong()
    }
}
